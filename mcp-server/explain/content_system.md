...
- `database.slug` — if present, the type is DB-backed (catalog-sourced). MCP `list_entries` / field read-update treat them like other entries (cache + overrides). MCP `create_entry` cannot create those rows. Do **not** confuse with `single_template: true` (e.g. static `blog`), which is YAML + shared `template.{locale}.yml` (legacy `single.*` still loads) and **is** creatable via `create_entry`.
...
- `editor.<field>.deprecated` (optional) — retire a field: `{ replaced_by: <field>|null, reason?, since? }` (`true` = no replacement). **Old entries** (live `_common.yml` or any live `{locale}.yml` already stores a non-empty value; DB-backed: the `field_overrides` bag) keep the value and may edit it. **New entries and entries without a value** cannot set it → `code: deprecated_field` with `deprecated { field, replaced_by, reason, field_path }`; `next_actions` → `update_entry_field` on `replaced_by` (empty when null). Clearing (null / empty) is always allowed; nothing is copied to the replacement. Enforced on every write path (update_entry_field, update_fields, edit-common, create_entry, duplicate strips it with `stripped_deprecated_fields`, raw file PUT, promote/publish of drafts). Not blocked: `field_mapping` defaults, DB column values, drafts that never go live. Config rules: cannot combine with `required` (`deprecated_config_invalid`); `replaced_by` must be a non-system, non-deprecated field on the type; a field named as someone's `replaced_by` cannot be removed or deprecated (`deprecated_replacement_target`). Adding a new `{{ entry.<field> }}` / `{{ single.<field> }}` ref to a deprecated field saves with warning `deprecated_template_ref`. **Read signals:** `get_content_type_info.deprecated_fields` + `deprecated_fields_note`; `get_entry_fields` catalog rows `deprecated: true`, `replaced_by`, `pick_hint`, value rows `deprecated` + `deprecated_locked` + system hint; `get_entry_content.deprecated_fields_present` (always an array). Write deprecation via `update_content_type` `field_action: update` + `editor.deprecated` (`null` restores); preview warns `deprecated_existing_values_stay` and `deprecated_template_refs` (count of files using the field). Staff: Content Type manage → Required-for-publish dialog → Retire.
- `layout.menu` — which navbar/footer menus to render
- `schema_org_requirements` (optional) — list of `{ schema_type }` companions every entry must have as a leading `schema_org` section (e.g. location → `LocalBusiness`, authors → `Person`). Validated by `schema-org-companions`; hard-gated on publish/promote / full locale replace (not on live micro structural saves). Coverage via `get_content_type_info`; day-to-day fix = Diagnostics (`SCHEMA_ORG_CONTENT_TYPE_REQUIREMENT`) + `add_section` with filled properties. `ensure_content_type_schema_org` is migration/bulk seed only (LocalBusiness from catalog / miami-usa|madrid-spain; other types often empty shells). Inspect resolved JSON-LD with `get_entry_seo` (not `get_entry_content`). Hero `course` variant separately requires a Course companion (`behaviors.schema_org.requires`).
...
SEO clustering, Search Console / Bing reads (`include_search_engines`), and live `meta.page_title` / `meta.description` gates → **`explain_site` topic `seo`**.

Page funnel stage / money pages → **`explain_site` topic `funnel`**.

### Required fields + live publish (non-SEO)
...
1. **`translate_entry`** in `attached_fields` mode — supply field_mapping keys (`title`/`content`/`bio`, …) + optional `meta`; optional top-level **`url_slug`** for this locale's public URL segment (do not pass `content.slug` or `content.url`). `sections` omit or `[]`. **Always writes a non-public variant** `{variant}.{locale}.yml` (default `variant: draft`) — never live `{locale}.yml`. Pass `variant` to choose the layer, or `create_variant: true` to seed a missing named layer from live (when live exists) then merge; default `draft` is auto-created when missing. Merge into an existing variant only when allocation is **0%** (or unregistered); allocation > 0 → `variant_has_traffic`. Optional `url_slug` is stored on the variant; uniqueness is validated at promote/publish. **`promote_variant` / `publish_draft` / propose `promote_on_apply`** go live. Draft may omit `editor.required` (warned). Shell still comes from `template.{locale}.yml`.
2. Edit draft (`get_entry_content` with `variant: draft`) → `run_entry_diagnostics` with `slugs: [slug]` and `freshness: "hard"` (returns `job_id` — poll `get_diagnostics_job` until completed) → **`promote_variant`** (one locale on a live entry) or **`publish_draft`** (all-draft entry). Confirm with the user before promote/publish.
3. After go-live success, follow the **required** `next_actions` entry: `run_entry_diagnostics` again (`hard` + one slug → poll `get_diagnostics_job`) to refresh the live validation cache (server does not auto-queue diagnostics on publish/promote).
