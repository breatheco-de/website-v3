...
- **Content types** — defined in `{content_folder}/content-types.yml`. Each type has a directory, URL pattern, and optional field mappings. **DB-backed** means `database.slug` is set — **list/read/update** via MCP the same as static types (`list_entries`, field tools); **create/delete** of those identities is blocked (`create_via: null`). **Shared-layout** means `single_template: true` and/or DB — shell lives in `template.{locale}.yml` (legacy `single.{locale}.yml` still loads). Example: `blog` is static YAML + `single_template` (not DB-backed).
